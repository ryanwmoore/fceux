...
function waitForUserUpload() {
    Module['addRunDependency'](RUNTIME_ROM_NAME);
}

function addUserData(arrayBuffer, file_name) {
    FS.writeFile(RUNTIME_ROM_NAME, new Uint8Array(arrayBuffer), {encoding: 'binary'});
...
$(function () {
    ...
    $(ROM_UPLOAD_IDENTIFIER).change(handleFileSelect);

    function prepareCommandLineArguments(string) {
        return string.split(" ");
    }

    function setCommandLine(evt) {
        Module.arguments = prepareCommandLineArguments($(this).val());
    }
